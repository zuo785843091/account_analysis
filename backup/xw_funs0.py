...
dict_col = {'A':0, 'B':1, 'C':0, 'D':1,'E':0, 'F':1,'G':0, 'H':1,'I':0, 'J':1,}
MAX_ROW_NUM = 5000
MAX_COL_NUM = 300
...
def update_account_analysis_table(wb, sht_name, dict_suppliers, sht_range = None):    
    ...
    for i in range(supplier_name_row + 1, last_row):
        supplier = arr[i, supplier_name_col].strip()
        values = dict_suppliers.get(supplier)
        
        # 清空遗留数据 J* : P*
        #clear_range = "J{r}:P{r}".format(r = i+1)
        #sht.api.Range(clear_range).ClearContents()
        #sht[i, int(stage_1_col)].value = [0] * 7
    ...
    '''
    #supplier_num = len(set(supplier_names))
    for supplier in dict_suppliers:
        index_s = np.argwhere(supplier_names == supplier)        
        
        values = dict_suppliers[supplier]
        non_verification_amount = []
        stage = []
        for v in values:
            aging_days = v[1]
        ...
        if len(index_s) == 0:            # 新增行
            ...
            for i in range(len(stage)):
                if sht[insert_row, int(stage_1_col + stage[i])].value != None:
                    nva = sht[insert_row, int(stage_1_col + stage[i])].value + non_verification_amount[i]
                else:
                    nva = non_verification_amount[i]
                sht[insert_row, int(stage_1_col + stage[i])].value = nva
                
            sht[insert_row, int(stage_1_col - 1)].value = sum(non_verification_amount)
            
            last_row += 1
            logging.info('新增：' + sht_name + ' ' + supplier)
            
        elif len(index_s) == 1:
            sht[int(index_s[0, 0]), int(stage_1_col)].value = np.array([None] * 7)
            for i in range(len(stage)):
                if sht[int(index_s[0, 0]), int(stage_1_col + stage[i])].value != None:
                    nva = sht[int(index_s[0, 0]), int(stage_1_col + stage[i])].value + non_verification_amount[i]
                else:
                    nva = non_verification_amount[i]
                sht[int(index_s[0, 0]), int(stage_1_col + stage[i])].value = nva
            """
            for s in set(stage):
                nva = 0
                for i in len(stage):
                    if s == stage[i]:
                        nva += non_verification_amount[i]
                sht[int(index_s[0, 0]), int(stage_1_col + s)].value = nva
            """
            sht[int(index_s[0, 0]), int(stage_1_col - 1)].value = sum(non_verification_amount)
    '''
def copy_last_account(wb, sht_name, sht_range = None):    
    ...
    try:
        ...
        logging.error("copy_last_account error: {e_info} \n \
                sht_name:{sht_name}".format(e_info = e, sht_name = sht_name))
        return False
...
if __name__ == '__main__':
    ...
    # 检查 ‘未核销’
    subjects, subjects_tables = get_subjects(wb1, sht_name = excel_configs.template_sheet, item_analysis = '未核销')
    
    for subject, subjects_table in zip(subjects, subjects_tables):
        subject = subject.strip()
        subjects_table = subjects_table.strip()
        dict_suppliers = check_non_verification_table(wb2, subject, sht_name = excel_configs.non_verification_sheet)
        if len(dict_suppliers) == 0:
            logging.warning("未在'未核销表'中查找到'{subject}'科目,请手动确认!!!".format(subject = subject))
            continue
        update_account_analysis_table(wb1, subjects_table, dict_suppliers)
        
        #copy_last_account(wb1, subjects_table)
    
    # 检查 ‘应付’
    subjects, subjects_tables = get_subjects(wb1, sht_name = excel_configs.template_sheet, item_analysis = '应付')
    for subject, subjects_table in zip(subjects, subjects_tables):
        subject = subject.strip()
        subjects_table = subjects_table.strip()
        dict_suppliers = check_payable_table(wb3, subject, sht_name = excel_configs.payable_sheet)
        if len(dict_suppliers) == 0:
